                    ...
                    <div class="card-body">
                        <form method="POST" action="" data-offline-table="payments" data-offline-action="update" data-offline-id="<?= $payment->id ?>">
                            ...
                            <div class="row form-row mb-3">
                                <div class="col-lg-6">
                                    <?= get_date_input('payment_date', old_value('payment_date',$payment->payment_date), 'Payment Date', ['class' => 'form-control']) ?>
                                </div>
                                <div class="col-lg-6">
                                   <label for="client">Client</label>
                                    ...
                                    <option value="">N/A</option>
                                        <?php if (!empty($clients)): foreach ($clients as $client): ?>
                                            <?php $selClient = old_value('client', $client->client_id) ?>
                            ...
                            <div class="row form-row mb-3">
                                <div class="col-lg-6">
                                    ...
                                    <input type="number" step="0.01" min="0" name="<?= esc('amount') ?>" value="<?= old_value('amount', $payment->amount) ?>" class="form-control" id="amount" placeholder="e.g. 500.00">
                                </div>
                                <div class="col-lg-6">
                                    <label for="pay_type">Payment Type</label>
                                    <?php $selPaymentType = old_value('pay_type', $payment->pay_type) ?>
                            ...
                            <div class="row form-row mb-3">
                                <div class="col-lg-6">
                                   <?= get_payment_type_dropdown('paid_via', old_value('paid_via', $payment->paid_via), 'Paid Via')  ?>
                                </div>
                                <div class="col-lg-6">
                                    <label for="notes">Notes (Optional)</label>
                                    <textarea name="<?= esc('notes') ?>" class="form-control" id="notes" rows="3"><?= old_value('notes', $payment->notes) ?></textarea>
                                </div>
                            </div>
                            ...
                            <div class="form-row">
                                <div class="d-grid gap-2 col-lg-12">
                                    <button type="submit" class="btn btn-outline-<?= THEME_COLOR ?>">UPDATE PAYMENT</button>
